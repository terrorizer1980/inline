bot = dofile('/home/sadra/inline/utils.lua')
json = dofile('/home/sadra/inline/JSON.lua')
...
BASE = '/home/sadra/inline/'
SUDO = 311704513 -- sudo id
sudo_users = {311704513,Userid}
...
---------------------reload -------------------------
       ...
       dofile('./cli.lua') 
 bot.sendMessage(msg.chat_id_, msg.id_, 1,'》 <code>💫Coder™©</code> <b>Reloaded  ✅</b>', 1, 'html')
            end

	   if text == 'reload inline' and is_sudo(msg) then
       dofile('./api.lua') 
 bot.sendMessage(msg.chat_id_, msg.id_, 1,'》 <code>💫Coder™© iNLiNE</code> <b>Reloaded  ✅</b>', 1, 'html')
            end
	    if text == 'CoderTeam' and is_sudoers(msg) then
    local gps = db:scard("botgp")
	local users = db:scard("usersbot")
    local allmgs = db:get("allmsg")

					bot.sendMessage(msg.chat_id_, msg.id_, 1, '➖➖➖➖➖➖➖➖➖➖\n`💫Coder™©` *V 2*✅\n\n*🌐Github Link 👇*\n\n`https://github.com/CoderTM/apiBot.git\n`\n➖➖➖➖➖➖➖➖➖➖\n_📊 BoT Stats 👇_\n\n*🌀 Super Groups 👉* `'..gps..'`\n*👤 Users 👉* `'..users..'`\n*📝 Message Received 👉* `'..allmgs..'`\n➖➖➖➖➖➖➖➖➖➖', 1, 'md')
	end
	  -----------------owner------------------------
      -- owner
